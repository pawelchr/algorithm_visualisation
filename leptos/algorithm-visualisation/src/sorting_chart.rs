...
use crate::sorting::{BarColor, MyData, SortRes};
use leptos::*;
...
//             <div class="items-center flex flex-auto justify-center basis-full">
//                 <BarChart
    ...
    let bars_data = Signal::derive(move || {current_data.with(|data| {
        data.iter()
            ...
            .map(|(index, step)| {
                ...
                Bar::new(move |_: &MyData| y1).with_colour(color).with_placement(BarPlacement::Zero).with_gap(0.1).with_group_gap(0.05)
            })
            .collect::<Vec<_>>()
    })});

    let series = Signal::derive(move || {Series::new(|step: &MyData| step.x).bars(bars_data().iter().cloned())});
